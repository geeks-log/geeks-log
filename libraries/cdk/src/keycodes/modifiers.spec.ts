import { createKeyboardEvent } from '../testing/fake-events';
import { hasModifierKey } from './modifiers';

describe('keycodes.modifiers', () => {
  ...
  it('should check if a particular modifier key is pressed', () => {
    const ctrlEvent = createKeyboardEvent('keydown', 0, '', undefined, { control: true });
    const ctrlAltEvent = createKeyboardEvent('keydown', 0, '', undefined, {
      control: true,
      alt: true,
    });
  ...
  it('should check if multiple specific modifier keys are pressed', () => {
    const ctrlEvent = createKeyboardEvent('keydown', 0, '', undefined, { control: true });
    const ctrlAltShiftEvent = createKeyboardEvent('keydown', 0, '', undefined, {
      control: true,
      alt: true,
      shift: true,
    });
